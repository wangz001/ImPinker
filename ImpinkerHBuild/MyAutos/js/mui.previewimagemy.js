(function($, window) {
	...
	var imgId = 0;
	var PreviewImage = function(options,funOpen,funClose) {
		...
		this.init();
		this.initEvent();
		this.funOpen=funOpen; //打开图片前后执行的事件
...
//		this.empty();
	...
	$.previewImage = function(options,funOpen,funClose) {
		if (!previewImageApi) {
			previewImageApi = new PreviewImage(options,funOpen,funClose);
		}
