...
(function() {
	...
	var starIndex = 0;
	var feedback = {
		Description: document.getElementById('question'),
		ContactWay: document.getElementById('contact'),
	...
	feedback.submitBtn.addEventListener('tap', function(event) {
		if(feedback.Description.value == '' ||
			(feedback.ContactWay.value != '' &&
				feedback.ContactWay.value.search(/^(\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+)|([1-9]\d{4,9})$/) != 0)) {
			return mui.toast('信息填写不符合规范');
		}
		if(feedback.Description.value.length > 200 || feedback.ContactWay.value.length > 200) {
		...
		feedback.send(mui.extend({}, feedback.deviceInfo, {
			...
			ContactWay: feedback.ContactWay.value,
